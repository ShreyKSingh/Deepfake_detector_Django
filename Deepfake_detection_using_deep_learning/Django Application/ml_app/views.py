    ...
    if request.method == "GET":
        ...
        try:
            ...
            for i in range(0, len(path_to_videos)):
                ...
                #request.session['original_video'] = original_video
                request.session['models_location'] = models_location
                request.session['output'] = output
                request.session['confidence'] = confidence
                request.session['video_file_name'] = video_file_name
                '''

            return render(request, predict_template_name, {'preprocessed_images': preprocessed_images, 'heatmap_images': heatmap_images, "faces_cropped_images": faces_cropped_images, "video_file_name": video_file_name, "models_location": models_location, "output": output, "confidence": confidence})
        except:
            return render(request, 'cuda_full.html')

def predict_for_api(request):
    err = "this is error"
    if request.method == 'GET':
        video_upload_form = VideoUploadForm()
        if 'file_name' in request.session:
        ...
        if video_upload_form.is_valid():
            ...
            file_name = os.path.join(settings.PROJECT_DIR, 'uploaded_videos', saved_video_file)
            #request.session['sequence_length'] = sequence_length

            path_to_videos = [file_name]
            video_file_name = video_file.name.split("\\")[-1]
            video_file_name_only = video_file_name.split('.')[0]
            video_dataset = validation_dataset(path_to_videos, sequence_length=sequence_length,transform= train_transforms)
            model = Model(2).cuda()
            model_name = os.path.join(settings.PROJECT_DIR,'models', get_accurate_model(sequence_length))
...
def json_file(request):
    ...
    video_file_name= request.session['video_file_name']
